...
| **17차 PDCA — tasks/issues 빈 상태 추론 버그 수정 (2026-05-04, 커밋 `83f0011`)** | ✅ **초급 사용자 빈 화면 UX 수정** — `tasks.tsx`: 0개 태스크 시 `'done'`→`'create'` (첫 태스크 생성 유도). `issues.tsx`: 0개 이슈 시 `'triage'`→`'open'` (이슈 등록 유도) / open 이슈 존재 시 `'open'`→`'triage'` (분류 작업 유도). 단위 테스트 시나리오 명칭도 의미론 정합 수정. FE 251/251 PASS. |
| **18차 PDCA — approvals 결재 플로우 단계 추론 정확도 개선 (2026-05-04, 커밋 `cd56699`)** | ✅ **상신 상태 추론 정확도 수정** — `counts.sent`(전체 상신) → `counts.pendingSent`(대기 중 상신만) 분리. 이미 결재된 상신도 `'submit'`으로 표시되던 버그 수정. `'archive'` 단계 추가(`history > 0 && pendingSent = 0` 시). 단위 테스트 4→5건. FE 251→252 PASS. |
| **19차 PDCA — progress/hr 화면 BusinessFlowStepper 추가 (2026-05-04, 커밋 `4004d95`)** | ✅ **화면 커버리지 9→11개 확장** — `progress.tsx`: `BUSINESS_FLOWS.project` 5단계 동적 추론(`plan/kickoff/execute/review/closeout`, done/blocked/doing 집계). `hr.tsx`: `BUSINESS_FLOWS.approval` 4단계 동적 추론(`draft/review/archive`, pendingLeaves/approvedLeaves 조건). FE **252/252** PASS. |
| 스펙 문서 드리프트 | 0건 — 19차까지 모두 반영. FE 252/252 · BE 692/692 현재 상태. |
