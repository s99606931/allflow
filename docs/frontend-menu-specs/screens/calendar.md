...
| 주간 뷰 | 주 단위 타임라인 | ✅ | 🔗 | 🧪 events.routes.test.ts |
| 일간 뷰 | 하루 단위 시간표 | ✅ | 🔗 | 🧪 events.routes.test.ts |
...
| 기능 | 설명 | 구현 | 백엔드 | 테스트 |
|------|------|------|--------|--------|
| 이전/다음 기간 | < > 버튼으로 기간 이동 | ✅ | 🔌 (쿼리 범위 변경) | 🧪 events.routes.test.ts |
...
| 이벤트 클릭 | 이벤트 상세 팝업 표시 | ✅ | 🔗 | 🧪 events.routes.test.ts |
