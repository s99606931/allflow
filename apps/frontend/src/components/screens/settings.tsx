...
function ProfileSection() {
  ...
  // Sync local form state when remote `me` resolves.
  useEffect(() => {
    if (me) {
      setName(me.name);
      setRole(me.role);
  ...
  return (
    <Section title="프로필" desc="동료들이 보는 프로필 정보입니다.">
        ...
        <CardBody className="space-y-1">
          <div className="flex items-center gap-4 pb-4">
            ...
            <div>
              ...
              {isLoading && <div className="text-[11px] text-fg-3 mt-1">불러오는 중...</div>}
      ...
      <div className="flex justify-end gap-2">
        <Button variant="secondary" onClick={onReset} disabled={update.isPending}>취소</Button>
