...
function makeMockPrisma() {
  const pins = new Map<
    string,
    { id: string; channelId: string; messageId: string; pinnedBy: string; createdAt: Date }
  >();
  return {
    channel: {
    ...
    pinnedMessage: {
      ...
      findUnique: async ({
        ...
        where: { channelId_messageId: { channelId: string; messageId: string } };
      }) => {
      ...
      upsert: async ({
        ...
        where: { channelId_messageId: { channelId: string; messageId: string } };
        create: { channelId: string; messageId: string; pinnedBy: string };
        update: object;
      }) => {
        const k = `${where.channelId_messageId.channelId}:${where.channelId_messageId.messageId}`;
...
describe('modules/channels — BE-N6', () => {
  ...
  it('POST /channels/:id/messages/:msgId/pin → 201 + pin id', async () => {
    ...
    const r = await app.inject({
      method: 'POST',
      url: '/channels/ch-general/messages/msg-test-1/pin',
      headers: { authorization: `Bearer ${u1}` },
    });
    expect(r.statusCode).toBe(201);
